...
public class SysLogAspect {//日志切面
     ...
     * @return
     * @throws Throwable
     * @Around注解修饰方法为一个环绕通知,其目的 是在目标业务方法执行之前和之后都可以进行
     * 扩展业务的处理
     * 其中:
     * 1)bean(yUserServiceImpl) 为切入点表达式,
     * 表示yUserServiceImpl对象中所有业务方法执行
     * 时都会执行@Around注解修饰的方法
     */
    //@Around("bean(sysUserServiceImpl)")
    //@Around("bean(*ServiceImpl)")
    //@annotation()为细粒度的切入点表达式定义方式
    @Around("@annotation(com.cy.common.annotation.RequiredLog)")
